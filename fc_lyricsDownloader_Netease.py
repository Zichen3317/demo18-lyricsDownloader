...
# 示例歌曲
    if Type == 'Netease_Song':  # 网易云音乐歌曲链接
        ...
        # 网易云音乐歌词api
        neteaseApiUrl_lyric = 'https://music.163.com/api/song/lyric?id=%s&lv=1&kv=1&tv=-1' % songID
        # 网易云音乐歌曲信息api
        neteaseApiUrl_detail = 'http://music.163.com/api/song/detail/?id=%s&ids=[%s]' % (
            songID, songID)
        ...
        try:
            ...
            songAuthorLst = []
            for i in myjson_detail['songs'][0]['artists']:
                songAuthorLst.append(i['name'])
            songAuthor = re.sub(
                r'[/]', chr(ord('/')+65248), '、'.join(songAuthorLst))
        ...
        try:
            ...
            limit = 300  # 歌单中歌曲信息获取数量限制

            # 网易云音乐歌单详细信息api
            neteaseApiUrl_playList = 'http://music.163.com/api/playlist/detail?id=%s&offset=0&total=true&limit=%s' % (
                playListID, limit)
            print(str(datetime.today()).split(' ')[
            ...
            print(myjson_playList)

            # 逐个获取歌单内的歌曲名及相应作者/翻唱者
            songLst = {}
            for songTotal in myjson_playList["result"]["tracks"]:
                # 歌曲名
                songName = songTotal['name']
                # 歌曲作者
                songAuthorLst = []
                for a in songTotal['artists']:
                    songAuthorLst.append(a['name'])
                songAuthor = '、'.join(songAuthorLst)
                # 将作者/翻唱者和歌曲名以字典形式存储
                songLst[songAuthor] = songName
